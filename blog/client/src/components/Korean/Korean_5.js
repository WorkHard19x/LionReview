    ...
    const uploadDate = new Date('2024-05-03'); // '2024-04-02' represents April 2, 2024
    ...
    const daysDifference = Math.floor(timeDifference / (1000 * 3600 * 24));
        ...
        const checkIfUserIsAdmin = () => {
            ...
            const storedIcons = localStorage.getItem('Korean_5_icons');
    ...
    useEffect(() => {
        ...
        localStorage.setItem('Korean_5_icons', JSON.stringify(icons));
    }, [icons]);
    ...
    const saveToMongoDB = async (icons, pageId) => {
        ...
        const userName = 'Korean_5-epdisode';
        const userName2 = 'Korean_5-image';
    ...
    const handleClose = () => {
        ...
        setTimeout(() => {
          setIsVisible(true);
                    ...
                    const storedImageData = localStorage.getItem('Korean_5-image');
                ...
                useEffect(() => {
                    // Save icons state to localStorage whenever it changes
                    localStorage.setItem('Korean_5-image', JSON.stringify(imageData));
                }, [imageData]);
                ...
                useEffect(() => {
                    ...
                    const fetchCurrentId = async () => {
                        try {
                            ...
                            console.log("Response from ID:", response.data[0]._id); // Log the entire response data

                            if (response.data.length > 0) {
                                console.log("Current ID:", response.data[0].date); // Access date from the first object in the array
                            } else {
                    ...
                    const updateDateInMongoDB = async () => {
                        try {
                            const response = await axios.put(`${API_BASE_URL}/api/get-id`, {
                                _id: '663f1f8f0a8018692c72379f', // current _id
                                date: '2024-05-20', // new date to be updated
                            });
