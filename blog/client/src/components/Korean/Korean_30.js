    ...
    const daysDifference = Math.floor(timeDifference / (1000 * 3600 * 24));
        ...
        const checkIfUserIsAdmin = () => {
            ...
            const storedIcons = localStorage.getItem('Korean_30_icons');
    ...
    useEffect(() => {
        ...
        localStorage.setItem('Korean_30_icons', JSON.stringify(icons));
    }, [icons]);
    ...
    const saveToMongoDB = async (icons, pageId) => {
        ...
        const userName = 'Korean_30-epdisode';
        const userName2 = 'Korean_30-image';
    ...
    const handleClose = () => {
        ...
        setTimeout(() => {
          setIsVisible(true);
                    ...
                    const storedImageData = localStorage.getItem('Korean_30-image');
                ...
                useEffect(() => {
                    // Save icons state to localStorage whenever it changes
                    localStorage.setItem('Korean_30-image', JSON.stringify(imageData));
                }, [imageData]);
                ...
                useEffect(() => {
                    ...
                    // const objectId = localStorage.getItem('id_data'); // Retrieve from local storage
                    ...
                    useEffect(() => {
                        const fetchIdData = async () => {
                            try {
                                const response = await axios.get('/api/kd/id_data');
                                const idDataList = response.data;
                                // Assuming you want to use the first id_data from the list
                                if (idDataList.length > 0) {
                                    setIdData(idDataList[0]);
                                }
                            } catch (error) {
                                console.error('Error fetching id_data:', error);
                            }
                        ...
                        fetchIdData(); // Call the function to fetch id_data when component mounts
                    }, []); // Empty dependency array ensures this effect runs only once
                
                    const { title } = useParams(); // Assuming `title` is obtained from URL params
                    ...
                    const [formData, setFormData] = useState({
                      ...
                      const fetchKoreanDramaByTitle = async (title) => {
                        try {
                            const response = await axios.get(`${API_BASE_URL}/api/kd/${title}`);
                            ...
                            console.error('Error fetching Korean drama by title:', error);
                        }
                    };
                    
                    // Call the function with the title parameter where needed
                    useEffect(() => {
                        // Assuming `title` is defined elsewhere, such as through useParams()
                        fetchKoreanDramaByTitle(title);
                    }, [title]);
                      ...
                      const handleSubmit = async (e) => {
                        e.preventDefault();
                        try {
                          // Make a PUT request to update the Korean drama data
                          await axios.put(`${API_BASE_URL}/api/kd/${title}`, formData);
                          alert('Korean drama updated successfully!');
                        } catch (error) {
                          console.error('Error updating Korean drama:', error);
                        }
                      };
  ...
  return (
    <div className="profile-container-headerd">
      ...
      <form onSubmit={handleSubmit}>
        ...
        {/* Add other fields as needed */}
        <button type="submit">Update Korean Drama</button>
      </form>
    </div>
