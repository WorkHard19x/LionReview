    ...
    const [uploadDate, setUploadDate] = useState(new Date('2024-05-09')); // Initialize upload date
    ...
    const daysDifference = Math.floor(timeDifference / (1000 * 3600 * 24));
        ...
        const checkIfUserIsAdmin = () => {
            ...
            const storedIcons = localStorage.getItem('Korean_10_icons');
    ...
    useEffect(() => {
        ...
        localStorage.setItem('Korean_10_icons', JSON.stringify(icons));
    }, [icons]);
    ...
    const saveToMongoDB = async (icons, pageId) => {
        ...
        const userName = 'Korean_10-epdisode';
        const userName2 = 'Korean_10-image';
    ...
    const handleClose = () => {
        ...
        setTimeout(() => {
          setIsVisible(true);
                    ...
                    const storedImageData = localStorage.getItem('Korean_10-image');
                    return storedImageData ? JSON.parse(storedImageData) : [
                        { url: "https://www.nettruyenvv.com/",
                          title: "Leo Doan",
                            img: "https://cdn.pixabay.com/photo/2022/12/01/04/35/sunset-7628294_640.jpg" },
                        { url: "",
                          title: "",
                            img: "" },
                        { url: "",
                ...
                useEffect(() => {
                    // Save icons state to localStorage whenever it changes
                    localStorage.setItem('Korean_10-image', JSON.stringify(imageData));
                }, [imageData]);
                ...
                //   const [kdIds, setKdIds] = useState([]);
                //   const [koreanDramaData, setKoreanDramaData] = useState([]);
                ...
                //         // Check if the response contains the kd_ids array
                //         if (response.data && response.data.kd_ids && response.data.kd_ids.length > 0) {
                //             // Assuming the current ID is the last one in the array
                //             const currentId = response.data.kd_ids[response.data.kd_ids.length - 1];
                //             setCurrentId(currentId); // Set the currentId state
                //             console.log("current id", currentId); // Log the currentId
                //         } else {
                ...
                //   const fetchKdIds = async () => {
                //     try {
                //         const response = await axios.get(`${API_BASE_URL}/api/get-id`);
                //         const kdIds = response.data.kd_ids;
                //         setKdIds(kdIds);
                //         console.log("kdIds",kdIds)
                
                //         console.log("pageId", pageId);
                //         if (Array.isArray(kdIds) && kdIds.length > 0) {
                //             console.log("kdIds[0]", kdIds[0]);
                //         }
                //         const index = kdIds.findIndex((id) => id === String(pageId));
                //         console.log("index", index);
                //         if (index !== -1) {
                //             setCurrentKdId(pageId);
                //         }
                //         console.log("current",currentKdId)
                //     } catch (error) {
                //         console.error('Error fetching Korean drama IDs:', error);
                //     }
                // };
                const [currentId, setCurrentId] = useState(null);
                ...
                const fetchCurrentId = async () => {
                    try {
                        const response = await axios.get(`${API_BASE_URL}/api/lastest_kd`);
                        if (response.data && response.data.current_id) {
                            setCurrentId(response.data.current_id);
                            console.log("Current ID:", response.data.current_id);
                        } else {
                            console.error('Invalid response format:', response.data);
                        }
                    } catch (error) {
                ...
                const updateDateInMongoDB = async (e) => {
                    try {
                        const newDate = new Date(e.target.value).toISOString().split('T')[0];
                        const response = await axios.put(`${API_BASE_URL}/api/get-id`, {
                            _id: currentId, // current _id
                            date: newDate, // new date to be updated
                        });
                        console.log(response.data.message); // Log the response message
                    } catch (error) {
                        console.error('Error updating date in MongoDB:', error);
                    }
                };
    ...
    <div className="profile-container-headerd">
        <div className="profile-containerd">
            ...
            <input
                type="date"
                id="uploadDate"
                value={uploadDate.toISOString().split('T')[0]} // Convert date to string in YYYY-MM-DD format
                onChange={(e) => updateDateInMongoDB(e)} // Define the event handler inline with the arrow function
            />
            ...
            <div className="profile-borderd">
                <div className="video-left">
                    <div className="vide-left-ads">
                        <div>
                            <p>h2</p>
                            {currentId && <p>Current _id: {currentId}</p>}

                            {/* {currentKdId && (
                        <p>Current KD ID: {kdIds[kdIds.findIndex((id) => id === currentKdId)]}</p>
                        )}
                        {currentKdId === null && (
                            <p>Current KD ID: {kdIds[pageId] || 'Not found'}</p>
                        )} */}
                            <p>h2</p>
                            <p>h2</p>
                            <p>h2</p>
